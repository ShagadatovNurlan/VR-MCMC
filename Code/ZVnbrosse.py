...
def ZVpolyOne(traj, traj_grad, f_target= "sum"):
    n, d = traj.shape
    if f_target == "sum":
...
def ZVpolyTwo(traj, traj_grad, f_target):
    n, d = traj.shape
    if f_target == "sum":
        samples = traj.sum(axis = 1).reshape(-1,1)
    elif f_target == "sum_squared":
...
def CVpolyOne(traj,traj_grad, f_target):
    n, d = traj.shape
    if f_target == "sum":
        samples = traj.sum(axis = 1).reshape(-1,1)
    elif f_target == "sum_squared":
...
def CVpolyTwo(traj, traj_grad, f_target):
    n, d = traj.shape
    if f_target == "sum":
        samples = traj.sum(axis = 1).reshape(-1,1)
    elif f_target == "sum_squared":
