...
\end{eqnarray*}
<<<<<<< HEAD
for all \(\mathbf{k}\) with \(1 <\|\mathbf{k}\|\leq  K+1\),\eric{I put \(1 <\|\mathbf{k}\|\leq  K+1\) instead of 
 \(0 <\|\mathbf{k}\|\leq  K+1\)...check :-)}
and that first order derivatives $\mu$ is bounded away from zero
\eric{this one really needed ? Strong convexity is not enough ?}
=======
for all \(\mathbf{k}\) with \(0<\|\mathbf{k}\|\leq  K+1\),
and the strong convexity assumption holds, that is,
>>>>>>> 274361ef27904c1b712d88c010f4e54a923deb1d
\[
...
\\
\lesssim K\left[\sum_{l=N+1}^{N+n} \sigma^2_l \right]
B
\left(\frac{BD+\log(2/\varepsilon)}{N_{tr}}+\frac{B^2D^2}{N_{tr}^2}\right)
\\
+
...
\begin{equation}
\label{eq:08062018b2}
\left|\partial_{y_l}^r G_{p,l}(x,y_l,\dots,y_p)\right| \le C_r\prod_{k=l+1}^p \alpha_k
\end{equation}
