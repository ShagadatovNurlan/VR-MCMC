...
\begin{equation}
\label{eq:a_est_int}
\widehat a_{l,k}(x)=\CPE{\phi_k\left(\xi\right)\EstFuncSmp{n}{l} \left(\Phi_l(x,\xi)\right)}{\TrainSet} \,,
\end{equation}
...
We focus on our  second algorithm where we estimate $\{ \EstFunc{l}{n} \}_{l=N}^{N+n}$ 
and its proof follows from  in \cite[Theorem~2.2]{audibert2011robust}.
...
\begin{eqnarray*}
r(\theta, x) = \frac{\rme^{\theta^T x}}{1 + \rme^{\theta^T x}}
\end{eqnarray*}
...
\begin{eqnarray*}
\pi(\theta) \propto \exp \left\{ -  \mathbf{Y}^T \mathbf{X \theta} - \sum_{i=1}^m \text{log} (1 + \rme^{-\mathbf{\theta}^T \mathbf{X}_i}) - \frac{\lambda}{2} \left\| \mathbf{\Sigma_X}^{1/2} \mathbf{\theta}\right\|_2^2 \right\}
\end{eqnarray*}
...
\begin{eqnarray*}
U( \mathbf{\theta}) = \mathbf{Y}^T \mathbf{X \theta} + \sum_{i=1}^m \text{log} (1 + \rme^{-\mathbf{\theta}^T \mathbf{X}_i}) + \frac{\lambda}{2} \left\| \mathbf{\Sigma_X}^{1/2} \mathbf{\theta}\right\|_2^2
\end{eqnarray*}
\begin{eqnarray*}
\nabla U(\mathbf{\theta}) = \mathbf{X}^T \mathbf{Y} - \sum_{i=1}^m \frac{\mathbf{X}_i}{1 + \rme^{\mathbf{\theta}^T \mathbf{X}_i}} + \lambda \mathbf{\Sigma_X \theta}
\end{eqnarray*}
