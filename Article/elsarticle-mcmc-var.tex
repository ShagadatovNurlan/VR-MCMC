...
\section{Practical implementation and complexity analysis for ULA}
\label{sec:coeff}
...
\end{equation*}
In both algorithms we first generate \(N_{tr}\)  paths conditionally independent of the burn-in sequence $X_1,\dots,X_N$ (hence, from the $\sigma$-algebra $\mathcal{G}_N$),  
\[
...
\begin{eqnarray*}
\widehat Q_{p,l}(x) = \sum_{\|\mathbf{s}\|\leq m} \hat{\beta}_{\mathbf{s}} x^{\mathbf{s}},\quad s=(s_1,\ldots,s_d)
\end{eqnarray*}
...
\begin{align*}
\widehat a_{p,l,\mathbf{k}} (x) =\CPE{\mathbf{H}_\mathbf{k}(x) \hat{Q}_{p,l}(x - \gamma \mu(x) + \sqrt{\gamma}\xi)}{\mathcal{D}_{tr} \vee \mathcal{G}_N} = \sum_{\|\mathbf{s}\|\leq m} \hat{\beta}_{\mathbf{s}} \prod_{i=1}^d E_i
\end{align*}
with
\begin{align*}
