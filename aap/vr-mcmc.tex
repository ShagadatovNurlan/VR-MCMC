...
\end{example}
\begin{example}
  Let \((X_t)_{t\in [0,T]}\) be the unique strong solution to the  SDE
\begin{eqnarray}
\label{eq:sde-inv}
dX_t=b(X_t)\,dt+\sigma(X_t)\,dW_t	
\end{eqnarray}
starting at \(X_0\) where \(W\) is a standard \(\mathbb{R}^m\)-valued Brownian motion, \(b:\) \(\mathbb{R}^d\to \mathbb{R}^d\) and \(\sigma:\) \(\mathbb{R}^d\times \mathbb{R}^m \to \mathbb{R}^d\) are locally Lipschitz continuous functions with at most linear growth.
...
As soon as there exists a continuously twice differentiable Lyapunov function \(V:\) \(\mathbb{R}^d\to \mathbb{R}_{+}\) such that 
\begin{eqnarray*}
\sup_{x\in \mathbb{R}^d} LV(x) <\infty,\quad \limsup_{|x|\to \infty} LV(x)<0,
\end{eqnarray*}
then there exists an invariant probability measure \(\pi\)  for \(X,\) that is, \(X_t\sim \pi\) for all \(t> 0\) if \(X_0\sim \pi.\) Invariant measures are crucial in the study of the long term behavior of stochastic differential
systems \eqref{eq:sde-inv}.   However, in many cases there is no explicit formula for the invariant measures and this makes the use of gradient based MCMC methods impossible in this case.  Under some additional assumptions,  the invariant
...
\end{eqnarray*}
Our method can be directly used to reduce variance of the ergodic estimator \(\pi_n^\gamma.\) 
\end{example}
