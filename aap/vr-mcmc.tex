...
For example, if \(\pi\) is analytically known and satisfies some regularity conditions, one can apply the well-known technique of  polynomial interpolation to construct control variates enjoying  some optimality properties, see, for example, Section~3.2 in \cite{dimov2008monte}. Alternatively, if an orthonormal system in \(L^2(\pi)\) is analytically available, one can build control variates \(\zeta\) as a linear combination of the corresponding basis functions, see \cite{GobetCV}. Furthermore, if \(\pi\) is known only up to a normalizing constant (which is often the case in Bayesian statistics), one can apply the recent approach of control variates  depending only on the gradient \(\nabla \log \pi\)  using Schr\(\ddot{\text{o}}\)dinger-type Hamiltonian operator in  \cite{mira2013zero} and so-called Stein operator in \cite{brosse2018diffusion}.
 ...
 \eric{mention the Stein operator}
...
\par
In this paper we focus on the  Langevin type algorithms which got much attention recently, see \cite{dalalyan2017theoretical,durmus:moulines:2017}
 \eric{add papers by Gilles Pages, Clement Rey, Vinvent Lemaire} 
...
\end{eqnarray*}
\end{example}
\erici{add here an example where $\pi$ is not explicitely known; tpicqlly we start from a diffusion $dX_t= b(X_t)dt + dW_t$ with $b$ Lipshitz and coercive. $\pi$ is not known but nevertheless we might be interested to estimate expectations under the stationary distribution of the diffusion... Our technique has no competitor in such problems. We can even be interested in estimating functionals of general Feller processes: see the recent results of Clement Rey, Gilles Pages, etc...}
...
\end{eqnarray*}
\eric{say here where the $\xi$ you are referreing to is defined}
Notice that this implies that the random variables
$\phi_k(\xi)$, $k\ge1$, are centered. As an example, we can take  multivariate Hermite polynomials for the ULA algorithm and product of Hermite polynomials for "gaussian part" and Shifted Legendre polynomials for "uniform part" of random variable for MALA, as the Shifted Legendre polynomials are orthogonal with respect to the Lebesgue measure on \([0,1].\) \eric{it is more tensor product than a product. and I need that perhaps some more precise notations are needed there}
...
$\mathsf{E}\left[\left|f(X_{p})\right|^{2}\right]<\infty$, where $X_{p}$ defined in \eqref{eq:chain_gen}. 
\eric{say where $X_p$ is defined... add a sentence like "where $X_p$ is defined in Eq. (xx)"}
Then,
...
is defined by
$$
R_\gamma(x,\cdot)=\mathcal{N}\left(x-\gamma\mu(x),\gamma\right),
$$
...
It follows from~\eqref{eq:29032018a5}, Theorem~\ref{prop:29032018a1} and independence condition of sequence $(Z_p)_{p \geq 1}$.
\begin{lem}
\label{cond_var_dec}
For any variance-reduced estimator~\eqref{eq:29032018a3}, defined for the Markov Chain~\eqref{eq:chain_gen} the following representation holds
...
\end{equation}
\end{lem}
\erici{this is not a direct consequence of Theorem !; this is of course true, no problem. But this remains valid more generally than for ULA.. this should be at least a Lemma explaining how you obtain this result}
...
\end{equation}
\eric{here $\xi$ is not defined. We should say that $\xi$ is a random variable independent from $\mathcal{T}$ with law $P_\xi$}
where $\Phi_l$ is defined in \eqref{eq:chain_gen} and the expectation is taken according to known distribution \(P_\xi$. Conditioning on $\TrainSet$ underlines that estimation of $\widehat Q_{l}$ derived from training trajectories. In many cases integration can be done in closed analytical form.
...
\end{align*}
is a one-dimensional polynomial (in \(x\)) of degree at most \(s\) with analytically known coefficients. Alternatively we can work directly with functions \(Q_{p,l}.\) Due to (almost) time-homogenous Markov chain generated by ULA, each function $Q_{p,l}$  depends only on  of the lag  $p-l$, that is,
\[
 Q_{p,l}(x) = \mathsf{E} \left[f(X_p) | X_l = x\right] = Q^\circ_{p-l}(x) 
 \]
Thus we have
\[ 
...
Due to Lemma~\ref{a_decay}, it is enough to estimate $Q^\circ_{r}$ for $|r| < n_{\mathrm{trunc}}$ for some truncation level \(n_{\mathrm{trunc}}\) depending on \(d\) and \(\gamma.\) It allows us to use a smaller amount of training trajectories (and even use only target trajectory) to approximate conditional expectations $Q^\circ_{r}$. 
...
\[
U(x) = \frac{1}{2} |x - a|^2 - \text{log}(1 + \rme^{-2x^\top a})
\]
and
\[
...
In order to approximate the expectation \(\pi(f)\) with \(f(x)=\sum_{i=1}^d x_i,\) 
 we have used constant step sizes $\gamma_i = \gamma=0.1.$   We generated $\NtrainPath = 50$ independent "training" trajectories and solved the least squares  problems \eqref{eq:Qcircr} using a first order polynomials for \(d=2\) and  \(d=8.\) The truncation level \(n_{\mathrm{trunc}}\) is chosen to be \(100.\) To test our variance reduction algorithm, we generated 100 independent trajectories. 
 
 \begin{figure}[tbh]
\centering
\subfloat[Subfigure 1 list of figures text][d = 2]{
\includegraphics[width=0.4\textwidth]{pictures/GM_2d_K1_n1000_deg1_ntrunc100_gamma01.png}
\label{fig:subfig1}}
\qquad
\subfloat[Subfigure 2 list of figures text][d = 8]{
\includegraphics[width=0.4\textwidth]{pictures/GM_8d_K1_n1000_deg1_ntrunc100_gamma01.png}
\label{fig:subfig2}}
\caption{Boxplots of $f(x) = \sum_{i=1}^d x_i$ using ULA algorithms for the Gaussian mixture model. The compared estimators are the ordinary empirical average  (pi), our estimator of control variates (CV-B), zero variance estimator (ZV-1) and control variates using diffusion approximation (CV-1). \label{fig:1}}
\end{figure}
