...
\end{align*}
is a one-dimensional polynomial (in \(x\)) of degree at most \(s\) with analytically known coefficients. 
...
Due to (almost) time-homogenous Markov chain generated by ULA, functions $Q_{p,l}$ can be approximated via regression depending only on number of lags $p-l$:
\[
...
\[ 
 ...
 \]
Consequently, the functions $Q^\circ_r$ can be estimated using the least squares minimisation
\begin{eqnarray}
\label{eq:Qcircr}
\widehat{Q}^\circ_r = \argmin_{\psi \in \Psi} \sum_{s = 1}^{T} \sum_{l = N + 1}^{N+n-r} \left| f(X_{l+r}^{(s)}) - \psi(X_l^{(s)})\right|^2 
\end{eqnarray}
for \(1 \leq r \leq n-1,\) where  $\widehat{Q}^\circ_0(x) = f(x).$
...
\sum_{l=N+1}^p {\widehat  a}_{p-l,k}(X_{l-1})\mathbf{H}_{\mathbf{k}}(Z_l) \mathbb{I}\{ |p-l| < n_{\mathrm{trunc}} \}. 
\]
In order to approximate the expectation \(\pi(f)\) with \(f(x)=\sum_{i=1}^d x_i,\) 
 we have used constant step sizes $\gamma_i = \gamma=0.1.$   We generated $\NtrainPath = 50$ independent "training" trajectories and solved the least squares  problems \eqref{eq:Qcircr} with polynomial basis functions of degree \(3\) for \(d=2\) and degree \(1\) for \(d=8.\) The truncation level \(n_{\mathrm{trunc}}\) is chosen to be \(100.\)
 
 \begin{figure}[tbh]
\centering
\subfloat[Subfigure 1 list of figures text][d = 2]{
\includegraphics[width=0.4\textwidth]{pictures/GM_2d_K1_n1000_deg1_ntrunc50_gamma01.png}
\label{fig:subfig1}}
\qquad
