...
\end{equation}
\eric{use only one $\xi$; change the notation of the control variable}
...
where $\Phi_l$ is defined in \eqref{eq:chain_gen}. In many cases be done in closed analytical form.
...
We use in this Section the notations and definitions of Example~\ref{exam:langevin-algorithm}.
...
and Theorem~\ref{prop:29032018a1}
...
\par
Upon estimating  the coefficients \((\widehat a_{l,k})\), one can construct the empirical version of \(M_{K,n}^N(f)\) in the form
\begin{eqnarray*}
\widehat M_{K,n}^N(f) =\sum_{1\leq  k \le K}\sum_{l=N+1}^{N+n}\widehat a_{l,k}(X_{l-1})\phi_k(\xi_{l}).
\end{eqnarray*}
Obviously \(\mathsf{E}[\widehat M_{K,n}^N(f)|\TrainSet]=0\) and \(\widehat M_{K,n}^N(f)\) is indeed a valid control variate in that it
